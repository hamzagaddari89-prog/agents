...
Section('B2 Generated-layer drift (real layer vs source)')
# Rebuild the rules block exactly as sync-rules.ps1 Build-Block does and
# compare it to every deployed generated block. Detection only - the remedy
# for drift is running sync-rules.ps1; this suite never repairs.
$rSTART = '<!-- shared-agents:rules:START (generated by sync-rules.ps1 - do not edit inside) -->'
$rEND   = '<!-- shared-agents:rules:END -->'
$rbParts = foreach ($f in ($rules | Sort-Object Name)) {
    "## $($f.BaseName)`n"
    (Get-Content $f.FullName -Raw).TrimEnd()
    ""
}
$rbBody = ($rbParts -join "`n").TrimEnd()
$rBlock = "$rSTART`n$rbBody`n$rEND"

function Get-DeployedBlock([string]$path) {
    if (-not (Test-Path $path)) { return $null }
    $raw = Get-Content $path -Raw
    $bi = $raw.IndexOf($rSTART); $ei = $raw.IndexOf($rEND)
    if ($bi -lt 0 -or $ei -lt 0) { return $null }
    return $raw.Substring($bi, $ei + $rEND.Length - $bi)
}
foreach ($t in @(
    ...
    $dep = Get-DeployedBlock $t.p
    T "$($t.n): deployed rules block matches source" ($null -ne $dep -and $dep -eq $rBlock)
}

# Generated agent files must match what sync-agents.ps1 generates (same
# content shape and TrimEnd comparison as the sync itself). Files without the
# generator marker are hand-placed and skipped, exactly like sync-agents does.
$gen = '<!-- GENERATED by ~\.agents\sync-agents.ps1 - safe to regenerate -->'
foreach ($rt in 'Claude', 'OpenCode') {
    $dir = if ($rt -eq 'Claude') { "$home_\.claude\agents" } else { "$home_\.config\opencode\agent" }
    $drifted = @()
    foreach ($f in (Get-ChildItem (Join-Path $root 'agents') -Filter '*.md' -File | Sort-Object Name)) {
        $raw = Get-Content $f.FullName -Raw
        $body = $raw.Substring($raw.IndexOf('---', 4) + 3).TrimStart()
        $meta = @{}
        ...
        if ($rt -eq 'Claude') {
            $expected = "---`nname: $($meta['name'])`ndescription: $($meta['description'])`n---`n`n$gen`n`n$body"
        } else {
            $expected = "---`ndescription: $($meta['description'])`nmode: subagent`n---`n`n$gen`n`n$body"
        }
        $dep = Join-Path $dir $f.Name
        if (-not (Test-Path $dep)) { $drifted += $f.Name; continue }
        if (-not (Select-String -LiteralPath $dep -SimpleMatch 'GENERATED by ~\.agents\sync-agents.ps1' -Quiet)) { continue }
        if ((Get-Content $dep -Raw).TrimEnd() -ne $expected.TrimEnd()) { $drifted += $f.Name }
    }
    T "${rt}: no drifted generated agents ($($drifted.Count))" ($drifted.Count -eq 0)
}
